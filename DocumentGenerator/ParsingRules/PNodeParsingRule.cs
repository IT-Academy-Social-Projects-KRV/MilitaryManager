    ...
    public class PNodeParsingRule : BaseParsingRule
    {
        ...
        public static PNodeParams GetAttributesData(XmlAttributeCollection attributes)
        {
            ...
            if (attributes != null)
            {
                ...
                if (fontStyleAttribute != null) Enum.TryParse(fontStyleAttribute.Value, true, out style);
                var fontSizeAttribute = attributes["fontSize"];
                if (fontSizeAttribute != null) fontSize = Convert.ToInt32(fontSizeAttribute.Value);
                var alignmentAttribute = attributes["alignment"];
                if (alignmentAttribute != null) Enum.TryParse(alignmentAttribute.Value, true, out alignment);
            }
