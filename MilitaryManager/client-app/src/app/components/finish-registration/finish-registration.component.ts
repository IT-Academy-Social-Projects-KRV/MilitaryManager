...
export class FinishRegistrationComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._attributeService.collection.getAll().subscribe((attributes)=>
    {
      ...
      this._attributeService.GetAttributeValues(this.attributes.filter(x=>x.name?.includes("Р“СЂСѓРїР° РєСЂРѕРІС–"))[0].id).subscribe((blood_types) => {this.blood_types = blood_types});
      this._attributeService.GetAttributeValues(this.attributes.filter(x=>x.name?.includes("РўРёРї С„РѕСЂРјРё"))[0].id).subscribe((uniforms) => {this.uniforms = uniforms});
    });
  }
